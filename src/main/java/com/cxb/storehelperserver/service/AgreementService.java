...
public class AgreementService {
    ...
    @Resource
    private AgreementBackRepository agreementBackRepository;
     ...
     * desc: 履约发货修改
     */
    public RestResult setShipped(int id, int oid, int sid, int aid, Date applyTime, List<Integer> commoditys, List<BigDecimal> prices,
     ...
     * desc: 线下销售修改
     */
    public RestResult setOffline(int id, int oid, int sid, int sid2, Date applyTime, List<Integer> commoditys, List<BigDecimal> prices,
                                 List<Integer> weights, List<String> norms, List<Integer> values, List<Integer> attrs) {
        // 已经审核的订单不能修改
        TAgreementOrder order = agreementOrderRepository.find(oid);
        if (null == order) {
     ...
     * desc: 履约退货
     */
    public RestResult backc(int id, TAgreementOrder order, List<Integer> commoditys, List<BigDecimal> prices, List<Integer> weights, List<Integer> values, List<Integer> attrs) {
        // 发货单未审核，已入库都不能退货
        int rid = order.getRid();
        TAgreementOrder storage = agreementOrderRepository.find(rid);
        if (null == storage) {
        ...
        // 生成退货单
        val comms = new ArrayList<TAgreementCommodity>();
        ret = createBackComms(order, rid, commoditys, prices, weights, values, comms);
        if (null != ret) {
            return ret;
        }
     ...
     * desc: 履约退货修改
     */
    public RestResult setBack(int id, int oid, Date applyTime, List<Integer> commoditys, List<BigDecimal> prices, List<Integer> weights, List<Integer> values, List<Integer> attrs) {
        // 已经审核的订单不能修改
        TAgreementOrder order = agreementOrderRepository.find(oid);
        if (null == order) {
            return RestResult.fail("未查询到要删除的订单");
        ...
        // 生成退货单
        val comms = new ArrayList<TAgreementCommodity>();
        ret = createBackComms(order, order.getRid(), commoditys, prices, weights, values, comms);
        if (null != ret) {
            return ret;
        }
        if (!agreementOrderRepository.update(order)) {
    ...
    public RestResult reviewBack(int id, int oid) {
        ...
        // 添加关联
        if (!agreementBackRepository.insert(oid, order.getRid())) {
            return RestResult.fail("添加履约退货信息失败");
        }
    ...
    public RestResult revokeBack(int id, int oid) {
        ...
        // 删除关联
        if (!agreementBackRepository.delete(oid)) {
            return RestResult.fail("删除采购退货信息失败");
        }
    ...
    private RestResult createBackComms(TAgreementOrder order, int aid, List<Integer> commoditys, List<BigDecimal> prices, List<Integer> weights, List<Integer> values, List<TAgreementCommodity> list) {
        // 生成发货单
        int size = commoditys.size();
        if (size != weights.size() || size != values.size()) {
            return RestResult.fail("商品信息异常");
