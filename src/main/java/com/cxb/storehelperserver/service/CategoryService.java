...
public class CategoryService {
    ...
    public RestResult getGroupCategory(int id) {
        ...
        if (null == categories) {
            return RestResult.fail("获取品类信息异常");
        }

        val datas = new ArrayList<HashMap<String, Object>>(); // 结果表，同时用来查找父节点
        for (TCategory c : categories) {
            ...
            // 从父类列表查找父类目，并插入
            for (HashMap<String, Object> p : datas) {
                if (p.get("id").equals(pid)) {
                    List<HashMap<String, Object>> children = (List<HashMap<String, Object>>) tmp.get("children");
                    if (null == children) {
