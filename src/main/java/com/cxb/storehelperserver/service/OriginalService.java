...
import static com.cxb.storehelperserver.util.TypeDefine.CommodityType.ORIGINAL;
...
public class OriginalService {
    ...
    public RestResult getStorageOriginal(int id, int sid, int page, int limit, String search) {
        ...
        for (TOriginal c : commodities) {
            ...
            // 库存
            TStockDay stock = stockService.getStockCommodity(gid, sid, ORIGINAL.getValue(), cid);
            if (null != stock) {
