 ...
 */
@Mapper
public interface MyStockWeekMapper {
    @Select({"<script>",
            "select count(t1.id) from t_stock_week t1",
    ...
    @Select({"<script>",
            ...
            "from t_stock_week t1 left join t_commodity t2 on t1.cid = t2.id",
            "where <if test='0 != gid'>t1.gid = #{gid}</if>",
            "<if test='0 == gid'>t1.sid = #{sid}</if> and t1.cdate = #{date}",
            "<if test='null != search'>and t2.name like #{search}</if>",
            "limit #{offset}, #{limit}",
    ...
    @Select({"<script>",
            "select sid as id, sum(value) as total, cdate from t_stock_week where gid = #{gid}",
