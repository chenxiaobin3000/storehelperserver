...
public interface MyStockCloudDayMapper {
    ...
    @Select({"<script>",
            "select t1.id, t1.gid, t1.aid, t1.cid, t1.price, t1.weight, t1.value, t2.code, t2.name, t2.cid as ctid, t2.remark",
    ...
    @Select({"<script>",
            ...
            "from t_market_commodity t1 left join (select id, gid, sid, cid, price, weight, value, cdate from t_stock_cloud_day",
            "where t1.aid = #{aid} and cdate = #{date}) t2 on t1.cid = t2.cid left join t_commodity t3 on t1.cid = t3.id",
            "where t1.aid = #{aid} <if test='null != search'>and t3.name like #{search}</if> limit #{offset}, #{limit}",
            "</script>"})
    List<MyStockCommodity> pagination_all(int offset, int limit, int aid, Date date, String search);

    @Select({"<script>",
            "select aid as id, sum(weight) as total, cdate from t_stock_cloud_day where aid = #{aid}",
