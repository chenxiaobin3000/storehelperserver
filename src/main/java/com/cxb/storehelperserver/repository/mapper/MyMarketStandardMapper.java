...
public interface MyMarketStandardMapper {
    @Select({"<script>",
            "select count(t1.id) from t_standard_storage t1 left join t_market_standard t2 on t1.cid = t2.cid",
            "and t1.sid = t2.sid where t1.sid = #{sid} <if test='null != search'>and t2.name like #{search}</if>",
            "</script>"})
    int count(int sid, String search);

    @Select({"<script>",
            "select t2.id, t2.sid, t2.mid, t2.aid, t2.asid, t1.cid, t2.code, t2.name, t2.remark, t2.price from t_standard_storage t1",
    ...
    @Select({"<script>",
            "select count(t1.id) from t_market_standard t1 left join t_market_standard_detail t2",
    ...
    @Select({"<script>",
            "select t2.id, t1.mid, t1.cid, t1.code, t1.name, t1.remark, t1.price as alarm, t2.price, t2.value from t_market_standard t1",
            "left join (select id, sid, aid, asid, cid, price, value, cdate from t_market_standard_detail where cdate = #{cdate})",
    ...
    @Select({"<script>",
            "select t2.cid, t2.code, t2.name, t2.remark, t1.price, t1.value from t_market_standard_detail t1",
            "left join t_market_standard t2 on t1.sid = t2.sid and t1.aid = t2.aid and t1.asid = t2.asid and t1.cid = t2.cid",
