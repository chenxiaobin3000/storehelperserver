...
public interface MyMarketStandardMapper {
    ...
    @Select({"<script>",
            ...
            "t3.code as code, t3.name as name, t3.price as price from t_market_standard t1",
            "left join (select id, cid, value, price from t_market_standard_detail where gid = #{gid} and mid = #{mid}",
            "and cdate = #{date}) as t2 on t1.cid = t2.cid left join t_standard t3 on t1.cid = t3.id",
