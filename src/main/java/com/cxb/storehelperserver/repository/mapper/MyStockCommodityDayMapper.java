 ...
 */
@Mapper
public interface MyStockCommodityDayMapper {
    @Select({"<script>",
            "select count(t1.id) from t_stock_commodity_day t1",
    ...
    @Select({"<script>",
            ...
            "from t_stock_commodity_day t1 left join t_commodity t2 on t1.cid = t2.id",
            "where t1.sid = #{sid} and t1.cdate = #{date}",
            "<if test='null != search'>and t2.name like #{search}</if>",
            "limit #{offset}, #{limit}",
            "</script>"})
    List<MyStockCommodity> selectBySid(int offset, int limit, int sid, Date date, String search);
    ...
    @Select({"<script>",
            ...
            "limit #{offset}, #{limit}",
            "</script>"})
    List<MyStockCommodity> selectByGid(int offset, int limit, int gid, Date date, String search);

    @Select({"<script>",
            "select sid as id, sum(value) as total, cdate from t_stock_commodity_day where gid = #{gid}",
