...
public interface MyStockCloudMapper {
    ...
    @Select({"<script>",
            "select aid as id, sum(value) as total from t_stock_cloud where gid = #{gid} and t1.aid = #{aid}",
    ...
    @Select({"<script>",
            ...
            "t2.code, t2.name, t2.cid as ctid, t2.remark from t_stock_cloud t1 left join t_commodity t2",
