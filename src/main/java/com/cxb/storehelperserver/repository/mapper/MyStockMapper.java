...
public interface MyStockMapper {
    @Select({"<script>",
            ...
            "</script>"})
    int count(int gid, int sid, Date start, Date end, String search);
    ...
    @Select({"<script>",
            ...
            "</script>"})
    List<MyStockCommodity> pagination(int offset, int limit, int gid, int sid, Date start, Date end, String search);
    ...
    @Select({"<script>",
            "select gid, sid, cid, sum(price) as price, sum(weight) as weight, sum(value) as value, date(cdate) as date",
