 ...
 */
@Mapper
public interface MyCloudWeekMapper {
    @Select({"<script>",
            "select count(t1.id) from t_cloud_week t1",
    ...
    @Select({"<script>",
            ...
            "from t_cloud_week t1 left join t_commodity t2 on t1.cid = t2.id",
            "where t1.gid = #{gid} <if test='0 != sid'>and t1.sid = #{sid}</if> and t1.cdate = #{date}",
            "<if test='null != search'>and t2.name like #{search}</if>",
            "limit #{offset}, #{limit}",
            "</script>"})
    ...
    @Select({"<script>",
            "select sid as id, sum(weight) as total, cdate from t_cloud_week where gid = #{gid}",
