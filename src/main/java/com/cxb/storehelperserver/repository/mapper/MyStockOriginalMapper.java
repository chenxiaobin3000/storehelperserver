 ...
 */
@Mapper
public interface MyStockOriginalMapper {
    @Select({"<script>",
            "select count(t1.id) from t_storage_original t1",
    ...
    @Select({"<script>",
            ...
            "limit #{offset}, #{limit}",
            "</script>"})
    List<MyStockOriginal> selectByExample(int offset, int limit, int sid, Date date, String search);
}
