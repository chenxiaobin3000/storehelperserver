 ...
 */
@Mapper
public interface MyStandardCloudMapper {
    @Select({"<script>",
    ...
    @Select({"<script>",
            "select t2.cid, t2.code, t2.name, t2.remark, t2.price from t_standard_cloud t1",
            "left join t_market_standard t2 on t1.cid = t2.cid where t1.sid = #{sid} and t2.mid = #{mid}",
